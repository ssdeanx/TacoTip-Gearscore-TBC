...
local INSPECTOR_MAX_CACHE = 150
local INSPECTOR_INSPECT_DELAY = 2
local INSPECTOR_MAX_QUEUE = 20
local INSPECTOR_REFRESH_DELAY = 10
local INSPECTOR_QUEUE_INTERVAL = 1
local INSPECTOR_INFO_MIN_INTERVAL = 5
...
local RegisterAddonMessagePrefix = (_G.C_ChatInfo and _G.C_ChatInfo.RegisterAddonMessagePrefix) or _G.RegisterAddonMessagePrefix or function()
    return true
end
    ...
    if (not cache.first) then
        ...
        if (cache.len >= INSPECTOR_MAX_CACHE) then
            if (IsInGroup() and IsGUIDInGroup(cache.first.guid)) then
                ...
                local nextNode = cache.first.next
                cache.first = nextNode
            end
        else
