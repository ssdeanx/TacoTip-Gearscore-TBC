...
-- load only on the Classic-era client families this library supports (Vanilla/TBC/Wrath)
...
local INSPECTOR_MAX_CACHE = 500
local INSPECTOR_INSPECT_DELAY = 1
local INSPECTOR_MAX_QUEUE = 20
local INSPECTOR_REFRESH_DELAY = 10
local INSPECTOR_QUEUE_INTERVAL = 1
local INSPECTOR_INFO_MIN_INTERVAL = 3
local INSPECTOR_INFO_MAX_INTERVAL = 60
...
local RegisterAddonMessagePrefix = (_G.C_ChatInfo and _G.C_ChatInfo.RegisterAddonMessagePrefix) or
    _G.RegisterAddonMessagePrefix or function()
        return true
    end
...
lib.cacheOrder = lib.cacheOrder or {}
...
local cacheOrder = lib.cacheOrder
...
local function addCacheUser(guid, inventory, talents, achievements, glyphs)
    ...
    cache[guid] = user
    tinsert(cacheOrder, guid)
    if (#cacheOrder > INSPECTOR_MAX_CACHE) then
        -- Evict old entries, protecting group members from being first evicted
        local evictIdx
        if (IsInGroup()) then
            for i, evictGuid in ipairs(cacheOrder) do
                if (not IsGUIDInGroup(evictGuid)) then
                    evictIdx = i
                    break
                end
            end
        end
        if (not evictIdx) then
            evictIdx = 1
        end
        local oldestGuid = tremove(cacheOrder, evictIdx)
        if (oldestGuid) then
        ...
        if (user) then
            if (refresh) then
                ...
                if ((not isClassic and user.talents.time < t) or user.inventory.time < t) then
                    ret = requestInspect(unit) or ret
                end
...
-- Anniversary-safe load-time hooks: only install detours when the target
-- function exists so client API drift cannot abort addon loading.
if (type(NotifyInspect) == "function") then
...
if (type(_G.PlayVocalErrorSoundID) == "function") then
...
function lib:CanInspect(unitorguid)
    ...
    return unit and UnitExists(unit) and UnitIsConnected(unit) and (not UnitIsDeadOrGhost(unit)) and
        ...
        (not InspectFrame or not InspectFrame:IsShown()) and type(CanInspect) == "function" and CanInspect(unit)
end
...
function lib:DoInspect(unitorguid)
    ...
    if (lib:CanInspect(unit)) then
        if (GetTime() >= nextInspectTime) then
            if (not requestInspect(unit)) then
                return 0
            end
            if (isWotlk) then
                tryCompare(unit)
...
function lib:GetInventoryItemMixin(unitorguid, slot)
    ...
    if (not Item or not Item.CreateFromEquipmentSlot or not Item.CreateFromItemID) then
        return nil
    end
    local n = tonumber(slot) or 0
    assert(n > 0 and n < 20, "inventorySlot is not a valid number (1-19)")
